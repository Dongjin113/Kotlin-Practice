...
//상속을 받을때는 변수명에서 한칸을 뛴후 : 를 붙혀서 사용한다 class Kotlin.Cat (species: String) : Kotlin.Animal{}
class Cat (
    species : String
) : Animal(species, 4){

    // cat 클래스를 만드는데 species라는 String을 받는 생성자가 같이생기고
    //Kotlin.Animal class를 상속 받고있는데 cat class를 만들어줄때 Kotlin.Animal class의 constructor를 불러주고
...
//2 인터페이스 Kotlin.Flyable 과 Swimmable 인터페이스
interface Swimable{
...
// Kotlin.Base class를 다른 클래스가 상속받을수 있도록 open으로 열어줬다
open class Base(
...
class Derived(
    ...
    init{
        //하위 Kotlin.Derived 클래스에서는 derived 클래스를 출력하고있다
